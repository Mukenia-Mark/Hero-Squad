...
public class Sql2oSquadDao implements SquadDao {
  ...
  @Override
  public void update(int id, String newName, String newCause, String newMembers){
    String sql = "UPDATE squad SET name = :name, cause = :cause, members = :members WHERE id=:id";
    try(Connection con = sql2o.open()){
      con.createQuery(sql)
         .addParameter("name", newName)
         .addParameter("cause", newCause)
         .addParameter("members", newMembers)
         .addParameter("id", id)
         .executeUpdate();
    } catch (Sql2oException ex) {
      System.out.println(ex);
